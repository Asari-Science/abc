...
function ThinkDirect(id) {
    ...
    if (id == 1) {
        ...
        if (pacDirect == Direct.None) // pacDirect == Direct.Noneのときは前回移動成功時の方向
            pacDirect = pacman.PrevDirect;
        if (pacDirect == Direct.North)
            monster.Direct = GetMonsterDirect(monster, pacX, pacY - 5, !monster.IsIjike);
        if (pacDirect == Direct.South)
            monster.Direct = GetMonsterDirect(monster, pacX, pacY + 5, !monster.IsIjike);
        if (pacDirect == Direct.East)
            monster.Direct = GetMonsterDirect(monster, pacX + 5, pacY, !monster.IsIjike);
    ...
    if (id == 2) {
        ...
        monster.Direct = GetMonsterDirect(monster, targetX, targetY, !monster.IsIjike);
    }
    if (id == 3) {
        // 距離の２乗
        // 遠ければ追い、近ければ逃げる
        let dis2 = Math.pow((monster.Position.CenterX - pacX), 2) + Math.pow((monster.Position.CenterY - pacY), 2);
        if (dis2 > 100)
            monster.Direct = GetMonsterDirect(monster, pacX, pacY, !monster.IsIjike);
        else
            monster.Direct = GetMonsterDirect(monster, pacX, pacY, monster.IsIjike);
...
function Draw() {
    ...
    if (showMonsterPoint) {
        ...
        con.fillText(addscore.toString(), monsterX * magnification + offsetX, monsterY * magnification + offsetY, 250);
    }
}
function DrawScore() {
    let strScore = "";
...
function DrawRest() {
    for (let i = 0; i < rest - 1; i++)
        con.drawImage(pacmanEastImage, offsetX + 110 + 25 * i, offsetY - 53, charctorSize * 0.7, charctorSize * 0.7);
}
// ドットに関すること
function CreateDots() {
    for (let x = 0; x <= 11; x++)
